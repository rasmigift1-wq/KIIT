...
export const getAQI = async (req, res) => {
  try {
    ...
    if (mockdata.temp <= 0) {
      mockdata.tdescription = "Freezing";
    } else if (mockdata.temp <= 15) {
      mockdata.tdescription = "Cold";
    } else if (mockdata.temp <= 25) {
      mockdata.tdescription = "Normal";
    } else if (mockdata.temp <= 35) {
      mockdata.tdescription = "Hot";
    } else {
      mockdata.tdescription = "Scorching";
    }
    mockdata.humidity = gettemp.data.main.humidity;
    if (mockdata.humidity <= 30) {
      mockdata.hdescription = "Low";
    } else if (mockdata.humidity <= 60) {
      mockdata.hdescription = "Normal";
    } else {
      mockdata.hdescription = "High";
    }
    ...
    if (mockdata.uvIndex <= 2) {
      mockdata.uvDescription = "Low";
    } else if (mockdata.uvIndex <= 5) {
      mockdata.uvDescription = "Moderate";
    } else if (mockdata.uvIndex <= 7) {
      mockdata.uvDescription = "High";
    } else {
      mockdata.uvDescription = "Very High";
    }
    res.status(200).json({
      status: 'success',
      data: mockdata
...
export const chatWithAI = async (req, res) => {
  try {
    ...
    if (!process.env.GEMINI_API_KEY) {
      console.warn("GEMINI_API_KEY not found in environment variables. Falling back to placeholder.");
      return res.status(200).json({
        reply: `This is a placeholder AI response to: "${message}". Please set GEMINI_API_KEY in the backend .env to enable true AI.`
      });
    }
    ...
    const model = genAI.getGenerativeModel({
      ...
      systemInstruction: "You are the ClimaCare AI assistant, an expert in environmental sustainability, carbon footprint reduction, and waste management. You help users navigate the ClimaCare dashboard, analyze their environmental metrics, and offer actionable advice to live more sustainably. Keep responses concise, supportive, and formatted cleanly. Do not use markdown headers unless necessary."
    });
